...
export default function Timer() {
  ...
  const formatMS = (duration: number) => {
    ...
    const leadingZero = (val: number) => (val > 9 ? val.toString() : '0' + val.toString())
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>): void => {
    ...
    const totalMilliseconds =
      parseInt(minutes || '00', 10) * 60000 + parseInt(seconds || '00', 10) * 1000
      ...
      <div className="flex h-full w-full flex-row items-start justify-center gap-2">
        <div className="flex flex-col items-center justify-center">
          <span className="countdown font-mono text-5xl">
            <span style={{ '--value': formatMS(count).split(':')[0] } as CSSProperties}></span>
          </span>
          min
        </div>
        ...
        <div className="flex flex-col items-center justify-center">
          <span className="countdown font-mono text-5xl">
            <span style={{ '--value': formatMS(count).split(':')[1] } as CSSProperties}></span>
          </span>
          sec
        </div>
      </div>
      <div className="flex flex-row items-center justify-center gap-4 pb-6">
        <div className="tooltip tooltip-bottom" data-tip={isPlaying ? 'stop' : 'start'}>
          <label
